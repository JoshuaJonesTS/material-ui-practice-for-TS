function MainSearch() {
    // 1. Loop through the list of objects and do the following on each object:

        // 1. Retrieve the value inside of the object that corresponds with the Search
        // Field specified by the user. (For example, if the search field is “age”, then
        // get the “age” value inside of the object)

        // 2. Check if the search value from the user matches the value inside of the object.

            // 1. If it matches, add the object to the search results list

            // 2. If it does not match, do nothing.

    // 2. Return the list of search results

    return (
        <div>

        </div>
    );
}

export default MainSearch;